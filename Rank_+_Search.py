...
with st.sidebar:
    ...
    try:
        defaultidx = [config_names].index(st.session_state.config["name"].lower())
    except:
        defaultidx = 0
    ...
    st.success("Select the configuration above.")

    
    search_options = ["Yes","No"]

    # Create a radio button in the sidebar and get the selected option
...
if len(prompt) > 0 and submitted == True:
    ...
    with st.spinner("Obtaining references from documents..."):
        ...
        retreived_keys = list(sources.keys())
        retrieved_docs = []
        for k,v in sources.items():
            retrieved_docs.append({"text":v['doc_content']})

        payload = {
            "model": "nv-rerank-qa-mistral-4b:1",
            "query":{"text":transformed_query["text"]},
            "passages":retrieved_docs
            }
        
        top_docs = llm_client.ranking_model(payload=payload)
        print("These are top docs",top_docs,"\n\n\n")
        updated_context = ""
        updated_sources = {}
        idx=0
        for m in top_docs:
